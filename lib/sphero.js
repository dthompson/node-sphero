...
SpheroCollection.prototype.connect = function(sphero_id) {
  var self = this;
    ...
    fs.readdir('/dev', function onDevicesListed(err, devs) {
      if (err) {
        ...
        devs.filter(function looksLikeSphero(dev) {
          var match = /cu.*sphero/i.test(dev);
          if( match && sphero_id ) {
            return -1 != dev.indexOf(sphero_id)
          }
          else return match;
...
SpheroCollection.prototype.setRotation = function(rate, callback) {
  this.balls.forEach(function(ball){
    ball.setRotation(rate, callback);
  });
  return this;
}
...
Sphero.prototype.setRotation = function setRotation(rate, callback) {
  var devrate = Math.max(255,Math.floor(rate / 0.784))
  console.log(devrate)
  //this._send(this.device.sphero.DID, this.device.sphero.commands.SET_ROTATION.CID, new Buffer([devrate & 0xff]), callback);
  return callback;
}
...
Sphero.prototype.configureLocator = function configureLocator(autoCorrect, x, y, yawtare, callback) {
  x = x || 0
  y = y || 0
  autoCorrect = !!autoCorrect
  yawtare     = yawtare || 0

  var data = new Buffer([autoCorrect ? 0x01 : 0x00, 
                         0x00,0x00,0x00,0x00,
                         0x00,0x00,0x00,0x00,
                         0x00,0x00,0x00,0x00,
                        ])

  data.writeInt16BE(x,1)
  data.writeInt16BE(y,5)
  data.writeInt16BE(yawtare,9)
  
  this._send(this.device.sphero.DID, this.device.sphero.commands.CONFIGURE_LOCATOR.CID, 
             data, callback);
  return callback;
}
